...
const API_TOKEN = process.env.API_TOKEN || 'HIDEBOTDOMPUL';
...
keyboard.push([{ text: '📦 Cek Paket XL', callback_data: 'cek_paket_xl' }]);
...
bot.action('cek_paket_xl', async (ctx) => {
  await ctx.answerCbQuery();
  const userId = ctx.from.id;

  userState[userId] = { state: 'menunggu_nomor_xl' };

  await ctx.reply('📲 Silakan kirim nomor XL yang ingin dicek (contoh: 087781777164):');
});
...
const OWNER_USERNAME = '@kytxz'; // Ganti ini dengan username kamu

function formatQuotaResult(text) {
    return text.replace(/<br>/g, '\n')
               .replace(/🎁 Quota:/g, '🎁 Quota:')
               .replace(/🍂 Aktif Hingga:/g, '🍂 Aktif Hingga:')
               .replace(/🎁 Benefit:/g, '🎁 Benefit:')
               .replace(/🎁 Tipe Kuota:/g, '🎁 Tipe Kuota:')
               .replace(/🎁 Kuota:/g, '🎁 Kuota:')
               .replace(/🌲 Sisa Kuota:/g, '🌲 Sisa Kuota:');
}

bot.on('text', async (ctx) => {
    const msisdn = ctx.message.text.trim();
    // Validasi nomor XL/AXIS
    if (!/^08\d{8,12}$/.test(msisdn) && !/^628\d{7,11}$/.test(msisdn)) {
        return ctx.reply(
            '❌ Mohon masukkan nomor XL/AXIS yang valid.\n\n✅ Contoh: *08123456789* atau *628123456789*',
            { parse_mode: 'Markdown' }
        );
    }

    try {
        // Panggil API sidompul v4 via POST
        const response = await axios.post(
            'http://sidompul.hidepulsa.com:34566/api/check',
            { msisdn },
            {
                headers: {
                    'Authorization': `Bearer HIDEBOTDOMPUL`,
        ...
        if (data.status) {
            // Jika struktur respons mengandung .data.hasil
            const formattedResult = formatQuotaResult(data.data.hasil || '');
            ctx.reply(
                `📡 HASIL CEK KUOTA 📡\n\n\`\`\`\n${formattedResult}\`\`\`\n${OWNER_USERNAME}`,
                { parse_mode: 'Markdown' }
            );
            ...
            ctx.reply(
                `❌ *Terjadi kesalahan:* ${data?.data?.keteranganError || 'Cek kuota gagal.'}`,
                { parse_mode: 'Markdown' }
            );
        }
    } catch (error) {
        console.error('❌ ERROR saat cek kuota:', error.message);
